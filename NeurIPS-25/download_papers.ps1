# NeurIPS 2025 Occupancy & Scene Completion Papers Download Script
# PDF 从 OpenReview 下载，需使用 Python openreview 客户端（因需认证/API）

$baseDir = "D:\develop\OccPaperCollect\NeurIPS-25\papers"
$indexPath = "D:\develop\OccPaperCollect\NeurIPS-25\paper_index.json"

if (!(Test-Path $indexPath)) {
    Write-Host "请先运行: python fetch_neurips25.py"
    exit 1
}
...
foreach ($p in $index) {
    ...
    $pdfFile = Join-Path $dir "$($p.short_title)_NeurIPS2025.pdf"
    ...
    try {
        ...
        if (Test-Path $metaPath) {
            ...
            $meta.local_pdf = "$($p.short_title)_NeurIPS2025.pdf"
