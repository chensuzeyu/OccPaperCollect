# ICLR 2026 Occupancy & Scene Completion Papers Download Script
# PDF 从 OpenReview 下载

$baseDir = "D:\develop\OccPaperCollect\ICLR-26\papers"
$indexPath = "D:\develop\OccPaperCollect\ICLR-26\paper_index.json"

if (!(Test-Path $indexPath)) {
    Write-Host "请先运行: python fetch_iclr26.py"
    exit 1
}
...
foreach ($p in $index) {
    ...
    $pdfFile = Join-Path $dir "$($p.short_title)_ICLR2026.pdf"
    ...
    try {
        ...
        if (Test-Path $metaPath) {
            ...
            $meta.local_pdf = "$($p.short_title)_ICLR2026.pdf"
